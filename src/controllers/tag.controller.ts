...
export default class TagController extends SequelizeController {
  ...
  public async listTagsForGroup(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const response = await this.tagService.listTagsForGroup(groupId, limit, page, req.query, sort);
  ...
  public async listTagsForImage(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const response = await this.tagService.listTagsForImage(imageId, limit, page, req.query, sort);
      res.status(HttpStatus.OK);
      res.json(response);
    } catch (error) {
      next(error);

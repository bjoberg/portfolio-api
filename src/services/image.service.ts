...
export default class ImageService extends SequelizeService {
   ...
   * @param sort sort order of request
   */
  public async listImagesNotForGroup(groupId: string, limit: number, page: number, filter: any, sort: string[] | undefined): Promise<PaginationResponse> {
    try {
      const offset = this.getOffset(limit, page);
      // @ts-ignore-next-line
      const result = await this.model.listImagesNotForGroup(groupId, limit, offset, filter, sort);
      const response: PaginationResponse = {
        limit,
        page,
        sort: result.sort,
